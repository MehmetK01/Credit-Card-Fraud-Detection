...
########################### too slow --> too much time #################################################
# Train KNN Model
set.seed(123)
knn_model <- train(Class ~ ., data = train_data_balanced, 
                   method = "knn", 
                   tuneLength = 10, 
                   trControl = control,
                   metric = metric)
...
# Evaluate on test data
knn_preds <- predict(knn_model, test_data, type = "prob")
knn_roc <- roc(test_data$Class, knn_preds[,2], levels = rev(levels(test_data$Class)))
knn_sensitivity <- sensitivity(predict(knn_model, test_data), test_data$Class)

# Print Results
cat("KNN Model - ROC:", knn_roc$auc, "Sensitivity:", knn_sensitivity, "\n")
#########################################################################################################
...
knn_model <- train(Class ~ ., 
                   ...
                   method = "knn", 
                   tuneLength = 5,            # Reduce k tuning grid to 5
